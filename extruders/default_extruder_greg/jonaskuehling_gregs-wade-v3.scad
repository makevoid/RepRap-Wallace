// Greg's Wade Extruder  2010 by GregFrost - http://www.thingiverse.com/thing:6713


// to check: filament_diameter 3 // 1.75
...
render() difference(){
  ...
  //wade(hotend_mount=jhead_mount, legacy_mount=false);
	...
	// 3dm1 - fan mount holes
	#translate([ 7-dist/2, 3.5, 0]) cylinder(15, m3_fan_mount/2, m3_fan_mount/2);
	#translate([ 7+dist/2, 3.5, 0]) cylinder(15, m3_fan_mount/2, m3_fan_mount/2);

}
//wade(hotend_mount=8, legacy_mount=false);
//wade(hotend_mount=jhead_mount, legacy_mount=false);
...
filament_feed_hole_d=2.3; // for 1.75
//filament_feed_hole_d=3.6; // for 3

filament_diameter=1.9; // for 1.75
//filament_diameter=3.2; // for 3
	...
	difference (){
		...
		rotate([-90,0,0]){
			...
			if (in_mask(hotend_mount,jhead_mount))
				wildseyed_mount_holes(insulator_d=15.7); // 3dm1 - default was 16
...
module groovemount_holes (){
	...
	//translate([0, 0, extruder_recess_h]) cylinder(r=3.8,h=1.7); // 3dm1 jhead screw space
}
